 ...
 echo "You must be root to run $0."
  ...
  then
   echo "Usage: $0 <username> <template_path> <num cores> <memory> <number of data interfaces> <vlan list> <data ip list> <vm name> <password>" 
    ...
    echo "Starting new VM for user $username, machine config $machine_confile, network config $network_confile, vm number $vm_name, num_cores=$num_cores"
...
echo "calling ./create_blank_vm.sh $vm_name $template_dir $num_cores $memory $number_of_interfaces $network_confile $username $data_ip_list $password"
